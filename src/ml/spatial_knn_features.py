...
class SpatialKNNFeatures:
    ...
    @classmethod
    def from_geojson(cls, geojson_path: str | Path) -> "SpatialKNNFeatures":
        """Load the SF reference pool from a GeoJSON FeatureCollection.

        Expects the same schema as sf_registered_businesses (i.e. the file
        produced by fetch-sf-registered-businesses.py or the SF open dataset).
        Status is derived using the same rules as _LABEL_SQL.
        """
        ...
        lats, lons = [], []
        dba_end_raw, loc_end_raw, admin_cl_raw, data_as_of_raw = [], [], [], []
        naic_codes, loc_starts, dba_starts = [], [], []
        ...
        for feat in data.get("features", []):
            ...
            dba_end_raw.append(props.get("dba_end_date"))
            loc_end_raw.append(props.get("location_end_date"))
            admin_cl_raw.append(props.get("administratively_closed"))
            data_as_of_raw.append(props.get("data_as_of"))
            naic_codes.append(props.get("naic_code") or None)
            loc_starts.append(props.get("location_start_date"))
            dba_starts.append(props.get("dba_start_date"))

        # Vectorized datetime parsing — ~100x faster than per-row Python calls
        now = pd.Timestamp.now(tz="UTC")
        six_months_ago = now - pd.Timedelta(days=182)

        dba_end    = pd.to_datetime(dba_end_raw,    utc=True, errors="coerce")
        loc_end    = pd.to_datetime(loc_end_raw,    utc=True, errors="coerce")
        data_as_of = pd.to_datetime(data_as_of_raw, utc=True, errors="coerce")
        admin_cl   = pd.array(admin_cl_raw, dtype=object)

        # Derive status (mirrors _LABEL_SQL)
        status = pd.array([None] * len(lats), dtype=object)
        status = np.where(dba_end.notna() & (dba_end <= now), "closed", status)
        status = np.where(admin_cl == "true", "closed", status)
        is_open = (
            dba_end.isna()
            & loc_end.isna()
            & pd.isna(admin_cl)
            & data_as_of.notna()
            & (data_as_of >= six_months_ago)
        )
        status = np.where(is_open & (status != "closed"), "open", status)
        # None entries remain None (uncertain)

        ref_df = pd.DataFrame({
            "lat":                 lats,
            "lon":                 lons,
            "status":              status,
            "naic_code":           naic_codes,
            "location_start_date": loc_starts,
            "dba_start_date":      dba_starts,
        })
        return cls(ref_df)

    @classmethod
    def from_file(cls, path: str | Path) -> "SpatialKNNFeatures":
        """Load from either a DuckDB (.ddb) or GeoJSON (.geojson/.json) file."""
        path = Path(path)
...
if __name__ == "__main__":
    ...
    featurizer = SpatialKNNFeatures.from_file(db)
